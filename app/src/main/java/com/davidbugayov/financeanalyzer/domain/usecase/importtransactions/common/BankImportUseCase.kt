        ...
        try {
            ...
            if (importedTransactions.isNotEmpty()) {
                ...
                try {
                    ...
                    Timber.i(
                        "[ИМПОРТ-ФИНАЛ] ✅ Эмитируем финальный результат: успешно импортировано $savedCount, пропущено $skippedCount, банк: $bankName",
                    )
                ...
                emit(
                    ImportResult.error(
                        "Неподдерживаемый формат файла. Пожалуйста, выберите CSV-файл или выписку из поддерживаемого банка.",
                    ),
                )
            }
        } catch (e: Exception) {
