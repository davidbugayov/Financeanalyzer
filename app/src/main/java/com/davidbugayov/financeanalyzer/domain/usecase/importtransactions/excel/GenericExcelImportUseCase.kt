...
class GenericExcelImportUseCase(
    ...
    private fun detectAlfaBankConfig(sheet: Sheet): ExcelParseConfig? {
        ...
        val headerCandidates = (0 until minOf(headerRowLimit, sheet.lastRowNum + 1)).mapNotNull { idx ->
            sheet.getRow(idx)
        }
     ...
     */
    private suspend fun extractDataFromExcel(uri: Uri, progressCallback: ImportProgressCallback): String = withContext(
        Dispatchers.IO,
    ) {
    ...
    override fun parseLine(line: String): Transaction? {
        ...
        try {
            // --- Дата ---
            val dateString = config.columnMapping.dateColumnIndex?.let {
            ...
            // --- Примечание (note) ---
            val note = config.columnMapping.noteColumnIndex?.let {
            ...
            // --- Сумма ---
            var amountString = config.columnMapping.amountColumnIndex?.let {
                columns.getOrNull(it)?.takeIf { it.isNotBlank() && it != "null" && it != "N/A" }
            }
            ...
            // --- Валюта ---
            val currencyString = config.columnMapping.currencyColumnIndex?.let {
                columns.getOrNull(it)?.takeIf { it.isNotBlank() }
            ...
            // --- Категория ---
            val rawCategory = config.columnMapping.categoryColumnIndex?.let {
                columns.getOrNull(it)?.takeIf { it.isNotBlank() }
            }
