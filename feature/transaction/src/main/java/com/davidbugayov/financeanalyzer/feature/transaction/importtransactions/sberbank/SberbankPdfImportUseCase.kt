...
class SberbankPdfImportUseCase(
    ...
    override fun isValidFormat(reader: BufferedReader): Boolean {
        ...
        val hasBankIndicator =
            textSample.contains("СБЕР", ignoreCase = true) ||
                textSample.contains(
                    "Сбербанк",
                    ignoreCase = true,
                )
    ...
    private fun createTransactionFromPartial(partial: PartialTransaction): Transaction? {
        try {
            ...
            val isExpense: Boolean =
                when {
                    ...
                    cleanedAmount.startsWith("+") -> false
                        ...
                        !(
                            partial.category?.contains("внесение наличных", ignoreCase = true) == true ||
            ...
            val detectedCategory =
                if (detectedCategoryFromBankCategory !=
                    com.davidbugayov.financeanalyzer.ui.R.string.no_category.let {
                        "Без категории"
                    }
                ) {
